...
void unescapeAtomLexeme(core::token::Lexeme &str) {
    if (!str.empty() && (str.front() == '|')) {         // multiescaped atom
        ...
        auto trash = std::remove_if(str.begin(), str.end(), [&escapeVisited](core::token::Lexeme::value_type c){
...
void Lexer::Private::refill(std::size_t n) {
    ...
    auto skippedHeadSize = std::distance(base, re2cState.lexemeStart);
